...
Y <- X + beta*log(xi^2) - beta*mean(log(xi^2))  # Log-transformation
```
...
u_l_theta <- function(y, phi, gamma, beta) {
  ...
  term2 <- exp(-1i * E * y) * (2^(1i * beta * y)) * gamma_z/sqrt(pi)
...
estimate_theta_qml <- function(y_t, initial_guess, beta) {
  result <- optim(par = initial_guess, fn = qml_log_likelihood, y_t = y_t, beta=beta, method = "L-BFGS-B", lower = c(-0.9, 0), upper = c(0.9, 1))
...
data_list <- list(
  ...
  beta = 1 / sqrt(5 * pi)  # Paramètre beta
)
